...
function findLastKeywordImage() {
    ...
    // DOMからメッセージが拾えない場合のSillyTavernContextフォールバック
    if (messages.length === 0 && typeof SillyTavern !== 'undefined' && SillyTavern.getContext) {
        const ctx = SillyTavern.getContext();
        if (ctx.chat && ctx.chat.length > 0) {
            const filteredChat = ctx.chat.filter(m => isUserMode ? m.is_user : !m.is_user);
            if (filteredChat.length > 0) {
                const lastMsg = filteredChat[filteredChat.length - 1];
                const mediaUrl = findMatchingImageUrl(lastMsg.mes);
